...
freya::PostProcessPass* vignette_pass_create(freya::Window* window, const freya::f32 intensity) {
  ...
  pass_desc.shader_context_id = asset_group_push_shader_context(pass_desc.asset_group, generate_vignette_shader());

  pass_desc.clear_flags = (freya::GFX_CLEAR_FLAGS_COLOR_BUFFER);
  pass_desc.debug_name  = "Vignette";

  pass_desc.attachments.emplace_back(freya::GFX_TEXTURE_FORMAT_RGBA8);
...
void vignette_pass_on_prepare(freya::PostProcessPass* pass) {
  ...
  freya::shader_context_set_uniform(pass->shader_context, "u_intensity", s_pass.intensity); 
