...
static void gl_error_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei len, const GLchar* msg, const void* usr_param) {
  switch(severity) {
    case GL_DEBUG_SEVERITY_HIGH: 
      FREYA_LOG_FATAL("GL-BACKEND: %s-%s (ID = %i): %s", gl_get_error_source(source), get_gl_error_type(type), id, msg);
      break;
    case GL_DEBUG_SEVERITY_MEDIUM: 
      FREYA_LOG_ERROR("GL-BACKEND: %s-%s (ID = %i): %s", gl_get_error_source(source), get_gl_error_type(type), id, msg);
      break;
    case GL_DEBUG_SEVERITY_LOW: 
      FREYA_LOG_WARN("GL-BACKEND: %s-%s (ID = %i): %s", gl_get_error_source(source), get_gl_error_type(type), id, msg);
      break;
    case GL_DEBUG_SEVERITY_NOTIFICATION: 
      FREYA_LOG_DEBUG("GL-BACKEND: %s-%s (ID = %i): %s", gl_get_error_source(source), get_gl_error_type(type), id, msg);
      break;
    default:
      break;
  }
...
GfxContext* gfx_context_init(const GfxContextDesc& desc) {
  ...
  if(!gladLoadGL()) {
    FREYA_LOG_FATAL("Could not create an OpenGL instance");
    return nullptr;
  }
  ...
  FREYA_LOG_INFO("An OpenGL graphics context was successfully created:\n" 
                 "              Vendor: %s\n" 
                 "              Renderer: %s\n" 
                 "              GL version: %s\n" 
                 "              GLSL version: %s\n" 
...
void gfx_context_use_bindings(GfxContext* gfx, const GfxBindingDesc& binding_desc) {
  ...
  for(sizei i = 0; i < binding_desc.textures_count; i++) {
    FREYA_DEBUG_ASSERT(binding_desc.textures[i], "An invalid texture found in texutres array");
...
GfxTexture* gfx_texture_create(GfxContext* gfx, const GfxTextureType tex_type, const AllocateMemoryFn& alloc_fn) {
  ...
  switch(tex_type) {
    ...
    case GFX_TEXTURE_DEPTH_STENCIL_TARGET:
      ...
      break;
    default:
      FREYA_LOG_ERROR("Invalid texture type");
      return nullptr;
  } 
