...
FREYA_API void input_init();

/// Update the internal state of the input system.
FREYA_API void input_update(); 

/// Returns `true` if `key` was pressed this frame.
FREYA_API const bool input_key_pressed(const Key key);

/// Returns `true` if `key` was released this frame.
FREYA_API const bool input_key_released(const Key key);

/// Returns `true` if `key` is currently held down.
FREYA_API const bool input_key_down(const Key key);

/// Returns `true` if `key` is currently held up.
FREYA_API const bool input_key_up(const Key key);

/// Returns `true` if the mouse `button` was pressed this frame.
FREYA_API const bool input_button_pressed(const MouseButton button);

/// Returns `true` if the mouse `button` was released this frame.
FREYA_API const bool input_button_released(const MouseButton button);

/// Returns `true` if the mouse `button` is currently held down.
FREYA_API const bool input_button_down(const MouseButton button);

/// Returns `true` if the mouse `button` is currently held up.
FREYA_API const bool input_button_up(const MouseButton button);

/// The current position of the mouse relative to the screen.
FREYA_API Vec2 input_mouse_position();
...
FREYA_API const bool input_cursor_on_screen();

/// Returns the _current_ connection status of the given joystick `id`.
FREYA_API const bool input_gamepad_connected(const JoystickID id);

/// Returns a number between `-1.0f` and `1.0f` for the joystick's axes or bumbers.
///
/// @NOTE: The X and Y of the resulting `Vec2` corresponds to the axis's directions if the given `axis` 
/// is either `GAMEPAD_AXIS_LEFT` or `GAMEPAD_AXIS_RIGHT`. However, for `GAMEPAD_AXIS_TRIGGER`, the X and Y 
/// correspond to the left (X) and right (Y) triggers/bumpers.
FREYA_API Vec2 input_gamepad_axis_value(const JoystickID id, const GamepadAxis axis);

/// Returns `true` if `button` of the gamepad `id` was pressed this frame.  
FREYA_API const bool input_gamepad_button_pressed(const JoystickID id, const GamepadButton button);

/// Returns `true` if `button` of the gamepad `id` was released this frame.  
FREYA_API const bool input_gamepad_button_released(const JoystickID id, const GamepadButton button);

/// Returns `true` if `button` of the gamepad `id` is being pressed.
FREYA_API const bool input_gamepad_button_down(const JoystickID id, const GamepadButton button);

/// Returns `true` if `button` of the jgamepad `id` is not pressed currently.
FREYA_API const bool input_gamepad_button_up(const JoystickID id, const GamepadButton button);

/// Get the name of the given gamepad `id` as a string if available.
FREYA_API const char* input_gamepad_get_name(const JoystickID id);

/// Bind the keys in `action` to the `action_name` for use later.
FREYA_API void input_action_bind(const String& action_name, const InputAction action);

/// Check if the binded keys for `action_name` are currently pressed.
FREYA_API const bool input_action_pressed(const String& action_name);

/// Check if the binded keys for `action_name` are currently released.
FREYA_API const bool input_action_released(const String& action_name);

/// Check if the binded keys for `action_name` are currently held down.
FREYA_API const bool input_action_down(const String& action_name);

/// Check if the binded keys for `action_name` are currently held up.
FREYA_API const bool input_action_up(const String& action_name);

/// Retrieve an `InputAction` struct with the name `action_name`.
FREYA_API const InputAction& input_get_action(const String& action_name);
