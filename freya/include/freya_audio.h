...
struct AudioListenerDesc {
  ...
  ///
  /// @NOTE: This is set to `{0.0f, 0.0f}` by default.
  Vec2 velocity = Vec2(0.0f);
};
/// AudioListenerDesc
///---------------------------------------------------------------------------------------------------------------------
...
/// default sound card.
FREYA_API bool audio_device_init(const char* device_name);

/// Shutdown the audio system, reclaiming any allocated memory.
FREYA_API void audio_device_shutdown();
...
FREYA_API AudioBufferID audio_buffer_create(const AudioBufferDesc& desc);

/// Destroy the given `buffer`, reclaiming any allocated memory in the process.
FREYA_API void audio_buffer_destroy(AudioBufferID& buffer);

/// Retrieve the underlying `AudioBufferDesc` of `buffer`.
FREYA_API AudioBufferDesc& audio_buffer_get_desc(AudioBufferID& buffer);

/// Update the data and parametars of `buffer` to the information provided in `desc`.
FREYA_API void audio_buffer_update(AudioBufferID& buffer, const AudioBufferDesc& desc);
...
/// an identifier for any future operations.
FREYA_API AudioSourceID audio_source_create(const AudioSourceDesc& desc);

/// Destroy the given `source`, reclaiming any allocated memory in the process.
FREYA_API void audio_source_destroy(AudioSourceID& source);

/// Retrieve the underlying `AudioSourceDesc` of `source`.
FREYA_API AudioSourceDesc& audio_source_get_desc(AudioSourceID& source);

/// Start/play the buffer(s) of the given `source`. 
///
/// @NOTE: If `source` currently lacks any audio buffers, this function 
/// will simply do nothing.
FREYA_API void audio_source_start(AudioSourceID& source);

/// Stop the processing of any buffer(s) of the given `source`. 
FREYA_API void audio_source_stop(AudioSourceID& source);
...
FREYA_API void audio_source_restart(AudioSourceID& source);

/// Pause any processing of buffer(s) of the given `source`.
FREYA_API void audio_source_pause(AudioSourceID& source);
...
FREYA_API void audio_source_set_pitch(AudioSourceID& source, const f32 pitch);

/// Set the looping flag of `source` to the given `looping`.
FREYA_API void audio_source_set_looping(AudioSourceID& source, const bool looping);

/// Set the position of `source` to the given `position`.
FREYA_API void audio_source_set_position(AudioSourceID& source, const Vec2& position);

/// Set the velocity of `source` to the given `velocity`.
FREYA_API void audio_source_set_velocity(AudioSourceID& source, const Vec2& velocity);

/// Set the direction of `source` to the given `direction`.
FREYA_API void audio_source_set_direction(AudioSourceID& source, const Vec2& direction);
...
FREYA_API void audio_listener_init(const AudioListenerDesc& desc);

/// Retrieve the underlying `AudioListenerDesc` of global audio listener.
FREYA_API AudioListenerDesc& audio_listener_get_desc();

/// Set the volume of global audio listener to the given `volume`.
FREYA_API void audio_listener_set_volume(const f32 volume);

/// Set the position of global audio listener to the given `position`.
FREYA_API void audio_listener_set_position(const Vec2& position);

/// Set the velocity of global audio listener to the given `velocity`.
FREYA_API void audio_listener_set_velocity(const Vec2& velocity);
